...
def is_metadata_table(table_node):
    ...
    # Define metadata keywords that indicate this is a document header/footer table
    metadata_keywords = [
        r'Novo\s+Nordisk',
        r'Trial\s+ID\s*:',
        r'Sample\s+eCRF',
        r'Mock-up',
        r'requirement',
        r'Version\s*:\s*\d+\.\d+',  # Version like 4.0
        r'Page\s*:\s*\d+\s+of\s+\d+',  # Page numbers like "9 of 118"
    ]
...
def is_instruction(text):
    ...
    keywords = re.compile(
        r'\b(please|note|ensure|click|enter|complete|select|indicate|check|provide|collect|integration|Study ID)\b',
        re.IGNORECASE)
